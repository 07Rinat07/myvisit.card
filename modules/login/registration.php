...
if (isset($_POST['register'])) {
    ...
    // Проверка на занятый емейл
    if (R::count('users', 'email = ?', array($_POST['email'])) > 0) {
        $errors[] = [
            'title' => 'Пользователь с таким Email уже зарегистрирован',
            'desc' => '<p>Используйте другой Email адрес, или воспользуйтесь <a href="' . HOST .
                'lost-password">восстановлением пароля</a>. </p>'
        ];
    }
    ...
    if (empty($errors)) {
       ...
       $result = R::store($user);
        ...
        if (is_int($result)) {
            ...
            $errors [] = ['title' => 'Что то пошло не так. Пожалуйста повторите еще раз'];
        }
    }
